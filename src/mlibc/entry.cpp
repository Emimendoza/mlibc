...
bool dynamically_linked_ = false;

const bool& mlibc::dynamically_linked = dynamically_linked_;


auxv_t *mlibc_auxv;

void mlibc::__mlibc_init(void* stack_pointer) {
	...
	page_size = static_cast<uint32_t>(page_size_entry->a_un.a_val);
	init_tls();
	call_init_funcs();
	atexit(call_fini_funcs);
	const int main_ret = main(info->argc, info->argv, environ);
	exit(main_ret);
}
...
const auxv_t* mlibc::getauxvect(const size_t type) {
